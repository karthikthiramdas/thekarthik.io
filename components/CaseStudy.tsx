    ...
    <main id="main-content">
      ...
      <section className="bg-ink text-paper section-y-lg">
        <div className="container-page">
          {data.hero.image ? (
            ...
            <div className="mb-12 border border-dashed border-paper/20 rounded-sm px-6 py-12 text-center">
          ...
          <div className="max-w-4xl">
            {data.hero.title.map((line, i) => (
              <h1
                key={i}
                className="font-display text-balance text-[clamp(2.75rem,7vw,5.5rem)] leading-[0.98] font-normal"
              >
                {line}
              </h1>
            ))}

            <p className="mt-8 max-w-2xl text-paper/60 text-lg leading-relaxed">
              {data.hero.subtitle}
            </p>
          </div>
        </div>
        ...
        return (
          ...
          >
            <div className="container-page">
              <Folio
                index={index}
                label={section.headline[0]}
              />

              {section.headline.length > 1 && (
                <Statement
                  lines={section.headline.slice(1)}
              ...
              {section.pending ? (
                <div className="mt-10 max-w-md">
                  <PendingSlot
                    label={section.pending.label}
                    note={section.pending.note}
                  />
                </div>
              ) : (
      ...
      <section className="bg-ink text-paper section-y">
        <div className="container-page">
          <Folio
            index="06"
            label={data.finalWork.headline[0]}
            dark
          />
          ...
          {data.finalWork.images &&
            data.finalWork.images.length > 0 && (
              <div className="mt-14 grid md:grid-cols-3 gap-8">
                {data.finalWork.images.map((img) => (
                  <figure key={img.src}>
                    ...
                    {img.caption && (
                      <figcaption className="mt-3 text-sm italic text-paper/50">
                        {img.caption}
                      </figcaption>
                    )}
                  </figure>
          ...
          {data.finalWork.reels &&
            data.finalWork.reels.length > 0 && (
